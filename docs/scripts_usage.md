...
> independent and CPU-bound (the NN runs on GPU, the PIBT sim on CPU). `--workers N` spreads them over a process pool — ~3.2× wall-clock at `--workers 8` for the
...
Per-episode reward = `success*w_success − makespan_norm*w_makespan − flowtime_norm*w_flowtime` (see `src/train/reward.py`). The resolved weights are
